    ...
    <AppShell>
      <div className="py-2">
        ...
        <div className="text-center mb-12">
          <span className="material-symbols-outlined text-4xl text-[var(--secondary-color)] mb-2 block">
            savings
          </span>
          <h1 className="text-4xl font-bold text-[var(--text-primary)] mb-3 font-display">
            Crowdfunding & Sponsorship Hub
          </h1>
          <p className="text-lg text-[var(--text-secondary)] max-w-2xl mx-auto">
            Empower artists to preserve heritage, fund grand performances, and sustain traditional
            crafts. Every contribution makes a difference.
          </p>
        </div>
          ...
          {[...campaigns]
            ...
            .map((c, i) => {
              ...
              return (
                <div
                  key={c._id || i}
                  className="bg-white rounded-2xl overflow-hidden card-shadow hover:-translate-y-1 transition-transform duration-300 group relative"
                >
                  ...
                  <div className="relative h-72 overflow-hidden">
                    ...
                    <div className="absolute bottom-4 left-4 flex gap-2">
                      {(c.tags || c.categories || []).map((tag, j) => (
                        <span
                          key={j}
                          className="bg-white/90 backdrop-blur-sm text-[var(--text-primary)] text-xs px-2 py-1 rounded-full font-medium"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="w-full bg-stone-100 rounded-full h-2.5 overflow-hidden">
                        <div
                          className={`h-full rounded-full ${percent >= 70 ? 'bg-green-500' : percent >= 40 ? 'bg-[var(--secondary-color)]' : 'bg-[var(--gold)]'}`}
                      ...
                      <div className="flex gap-4 text-stone-500">
                        <span>
                          <strong className="text-[var(--text-primary)]">
                            {c.backerCount ?? c.backersCount ?? 0}
                          </strong>{' '}
                          backers
                        </span>
                        <span>
                          <strong className="text-[var(--text-primary)]">{c.daysLeft ?? 0}</strong>{' '}
                          days left
                        </span>
                      </div>
                      {user && c.creator?._id !== user._id && (
                        <Button
                          onClick={() => toggleBackingForm(c._id)}
                          className="bg-[var(--primary-color)] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-[var(--secondary-color)] transition-colors shadow-md"
                        >
                    ...
                    {backingActive.has(c._id) && (
                      <div className="mt-3 flex gap-2 items-center">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setBackingAmounts((prev) => ({ ...prev, [c._id]: e.target.value }))
                          }
                        ...
                        >
                          {backingLoading[c._id] ? 'Processing...' : 'Contribute'}
                        </Button>
                      </div>
                    )}
                  </div>
        ...
        {user?.role === 'artist' && (
          <div className="text-center mb-12">
            <Link
              ...
              className="bg-[var(--primary-color)] text-white px-8 py-3 rounded-full font-bold hover:bg-[var(--secondary-color)] transition-colors shadow-lg inline-flex items-center gap-2"
            >
