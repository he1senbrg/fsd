...
import { Button, FormInput, FormSelect, FormStepper, FormTextarea, Loader, SurfaceCard } from "@/components/ui";
...
export default function CreateCampaignPage() {
    ...
    const validateStep = () => {
        if (step === 1) {
            if (!form.title.trim()) { showToast("Campaign title is required.", "warning"); return false; }
            if (!form.shortDescription.trim()) { showToast("Short description is required.", "warning"); return false; }
        }
        if (step === 2) {
            if (!form.fullStory.trim()) { showToast("Full story is required.", "warning"); return false; }
            if (!form.coverImage) { showToast("Please upload a cover image.", "warning"); return false; }
        }
        if (step === 3) {
            if (!form.goalAmount || Number(form.goalAmount) < 1) { showToast("Please enter a valid funding goal.", "warning"); return false; }
        }
        return true;
    };
        ...
        return (
            <AppShell>
                <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
                    ...
                    <Button onClick={() => router.push("/crowdfunding")} className="mt-2 bg-[var(--primary-color)] text-white px-6 py-2 rounded-full font-bold">
                        Back to Campaigns
                    </Button>
                </div>
            </AppShell>
    ...
    return (
        <AppShell>
            <div className="py-2">
                ...
                <FormStepper
                    currentStep={step}
                    steps={["Project Info", "Story & Media", "Funding Goal"]}
                />
                ...
                <SurfaceCard className="p-4 sm:p-6 md:p-8">
                                ...
                                >
                                    {imagePreview ? (
                                        <div className="relative">
                                            ...
                                            <img src={imagePreview} alt="Cover preview" className="w-full h-64 object-cover rounded-xl" />
                                        ...
                                        <>
                                            ...
                                            <p className="text-sm text-stone-500">{imageUploading ? "Uploading..." : "Click to upload cover image"}</p>
                    ...
                    {step === 3 && (
                        <div className="space-y-6">
                            ...
                            <div>
                                <label className="text-sm font-semibold text-[var(--text-primary)] mb-1 block">Tags <span className="font-normal text-stone-400">(comma separated)</span></label>
                                <FormInput
                                ...
                                {form.tags && (
                                    <div className="flex flex-wrap gap-1.5 mt-2">
                                        {form.tags.split(',').map(t => t.trim()).filter(Boolean).map((tag, i) => (
                                            <span key={i} className="bg-orange-100 text-[var(--primary-color)] text-xs px-2.5 py-0.5 rounded-full border border-orange-200 font-medium">{tag}</span>
                                        ))}
                                    </div>
                                )}
                            </div>
                            <div className="bg-orange-50 rounded-xl p-4 border border-orange-100">
                                <h4 className="text-sm font-semibold text-[var(--text-primary)] mb-1 flex items-center gap-1">
                                    <span className="material-symbols-outlined text-sm text-[var(--terracotta)]">info</span>
                                    Summary
                                </h4>
                                <ul className="text-sm text-stone-600 space-y-1 mt-2">
                                    <li><strong>Title:</strong> {form.title || "—"}</li>
                                    <li><strong>Goal:</strong> {form.goalAmount ? `₹${Number(form.goalAmount).toLocaleString("en-IN")}` : "—"}</li>
                                    <li><strong>Duration:</strong> {form.duration} days</li>
