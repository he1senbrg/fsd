        ...
        <AppShell>
            <div className="py-2">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-orange-100 p-4 mb-8 flex flex-col md:flex-row gap-4">
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            placeholder={roleFilter === "artLover" ? "Search by name or bio..." : "Search by name, skill, or art form..."}
                            type="text"
                        />
                    </div>
                    <select
                ...
                {filteredArtists.length === 0 && (
                    <EmptyState description={roleFilter === "artLover" ? "No art lovers found." : "No artists found."} icon="person_search" />
                )}
                            ...
                            <div className="p-5">
                                <div className="flex items-start justify-between mb-2">
                                    <div>
                                        ...
                                        <p className="text-sm text-[var(--text-secondary)]">{artist.role === "artLover" ? "Art Lover" : artist.title || artist.primaryArtForm || "Artist"}</p>
                                    </div>
                                    {roleFilter === "artist" ? (
                                ...
                                <div className="flex items-center gap-2 text-xs text-stone-500 mb-2">
                                    <span className="material-symbols-outlined text-sm">location_on</span>{artist.location || "India"}
                                ...
                                <div className="grid grid-cols-2 gap-2" onClick={(e) => e.preventDefault()}>
                                    ...
                                    <Button variant="primary" onClick={(e) => { e.preventDefault(); e.stopPropagation(); router.push(roleFilter === "artist" && artist._id ? `/profile?id=${artist._id}&book=true` : artist._id ? `/profile?id=${artist._id}` : '/profile'); }} className="text-xs shadow-md">{roleFilter === "artist" ? "Book Now" : "Open Profile"}</Button>
                                </div>
                            </div>
                        </Link>
                    ))}
