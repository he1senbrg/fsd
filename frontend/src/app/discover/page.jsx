...
function DiscoverContent() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const res = await artistAPI.getArtists({ role: roleFilter });
  ...
  useEffect(() => {
    if (roleFilter === 'artLover') {
      setArtForm('All Art Forms');
    }
  }, [roleFilter]);
  ...
  const filteredArtists = artists.filter((artist) => {
    ...
    return matchesQuery && (roleFilter === 'artLover' ? true : matchesArtForm);
  });
  ...
  return (
    <AppShell>
      <div className="py-2">
        <PageHeader
          title="Discover Artists & Art Lovers"
          subtitle="Browse profiles, explore the community, and connect with talented people preserving culture."
        />
            ...
            <input
              ...
              placeholder={
                roleFilter === 'artLover'
                  ...
                  : 'Search by name, skill, or art form...'
              }
              type="text"
            />
          </div>
        ...
        {filteredArtists.length === 0 && (
          <EmptyState
            description={roleFilter === 'artLover' ? 'No art lovers found.' : 'No artists found.'}
            icon="person_search"
          />
        )}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredArtists.map((artist, i) => (
            <Link
              key={artist._id || i}
              href={artist._id ? `/profile?id=${artist._id}` : '/profile'}
              className="bg-white rounded-xl overflow-hidden card-shadow hover:-translate-y-1 transition-transform group block"
            >
              ...
              <div className="p-5">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h3 className="font-bold text-lg text-[var(--text-primary)] font-display">
                      {artist.name || artist.fullName}
                    </h3>
                    <p className="text-sm text-[var(--text-secondary)]">
                      {artist.role === 'artLover'
                        ? 'Art Lover'
                        : artist.title || artist.primaryArtForm || 'Artist'}
                    </p>
                  </div>
                  {roleFilter === 'artist' ? (
                    <div className="flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded text-xs">
                      ...
                      <span className="text-stone-400">
                        ({artist.reviews || artist.reviewCount || 0})
                      </span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-1 bg-orange-50 px-2 py-1 rounded text-xs text-[var(--primary-color)]">
                      <span className="material-symbols-outlined text-sm">groups</span>
                      <span className="font-bold">{artist.followerCount || 0}</span>
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-2 text-xs text-stone-500 mb-2">
                  ...
                  {roleFilter === 'artist' && (artist.price || artist.startingPrice) && (
                    <>
                      <span className="mx-1">•</span>
                      <span className="font-bold text-[var(--primary-color)]">
                        {artist.price || artist.startingPrice}
                      </span>
                    </>
                  )}
                </div>
                {roleFilter === 'artist' && (
                  <div className="flex gap-2 flex-wrap mb-4">
                    {(artist.tags || artist.specializations || []).map((tag) => (
                      <span
                        key={tag}
                        className="bg-orange-50 text-[var(--primary-color)] text-xs px-2 py-0.5 rounded-full border border-orange-200"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="grid grid-cols-2 gap-2" onClick={(e) => e.preventDefault()}>
                  <Button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      router.push(artist._id ? `/profile?id=${artist._id}` : '/profile');
                    }}
                    className="text-xs"
                  >
                    View Profile
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      e.stopPropagation();
                      router.push(
                        roleFilter === 'artist' && artist._id
                          ? `/profile?id=${artist._id}&book=true`
                          : artist._id
                            ? `/profile?id=${artist._id}`
                            : '/profile',
                      );
                    }}
                    className="text-xs shadow-md"
                  >
                    {roleFilter === 'artist' ? 'Book Now' : 'Open Profile'}
                  </Button>
                </div>
              </div>
            </Link>
