...
function ProfileContent() {
    ...
    async function load() {
      ...
      try {
        ...
        if (portfolioRes.status === 'fulfilled')
          setPortfolio(portfolioRes.value.data?.posts || portfolioRes.value.data?.portfolio || []);
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      setReviews((prev) => [
        res.data.review,
        ...prev.filter((r) => r.reviewer?._id !== authUser?._id),
      ]);
      setReviewRating(0);
      setReviewText('');
      // update local rating
  ...
  const toggleLike = async (postId) => {
    ...
    setPortfolio((prev) =>
      prev.map((p) =>
        p._id === postId
          ? { ...p, isLiked: !wasLiked, likeCount: (p.likeCount || 0) + (wasLiked ? -1 : 1) }
          : p,
      ),
    );
    try {
  ...
  const sharePost = async (postId) => {
    try {
      ...
      await Promise.all([postAPI.sharePost(postId), navigator.clipboard.writeText(postUrl)]);
      ...
      setPortfolio((prev) =>
        prev.map((p) => (p._id === postId ? { ...p, shareCount: (p.shareCount || 0) + 1 } : p)),
      );
  ...
  const lightboxNext = () =>
    setLightbox((lb) => (lb ? { ...lb, index: (lb.index + 1) % lb.images.length } : null));
  const carouselPrev = (id, len) =>
    setCarouselIndex((prev) => ({ ...prev, [id]: ((prev[id] ?? 0) - 1 + len) % len }));
  const carouselNext = (id, len) =>
    setCarouselIndex((prev) => ({ ...prev, [id]: ((prev[id] ?? 0) + 1) % len }));
      ...
      <div className="py-2">
            ...
            <div className="absolute bottom-0 left-0 right-0 px-8 py-6 bg-gradient-to-t from-black/50 to-transparent hidden md:flex items-end gap-6">
              <div className="w-32 h-32 rounded-full border-4 border-white overflow-hidden shadow-xl -mb-2">
                <Image
                  alt={u.fullName || 'User'}
                  className="w-full h-full object-cover"
                  src={u.avatar || '/avatar-placeholder.svg'}
                  width={128}
                  height={128}
                  unoptimized
                />
              ...
              <div className="text-white flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {u.isVerified && (
                    <span className="material-symbols-outlined text-blue-400 text-xl filled">
                      verified
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 mt-2 text-sm text-white/70">
                  ...
                  <span className="flex items-center gap-1">
                    <span className="material-symbols-outlined text-sm">calendar_today</span>Member
                    since {memberSince}
                  </span>
                </div>
              </div>
          ...
          <div className="md:hidden -mt-12 relative z-10 mx-4">
            <div className="bg-white rounded-2xl shadow-lg border border-orange-100 px-5 pt-14 pb-5 text-center relative">
              ...
              <div className="flex items-center justify-center gap-2 mb-1">
                ...
                {u.isVerified && (
                  <span className="material-symbols-outlined text-blue-500 text-lg filled">
                    verified
                  </span>
                )}
              </div>
              <p className="text-[var(--text-secondary)] text-sm mb-2">
                {u.title || u.primaryArtForm || ''}
              </p>
              <div className="flex items-center justify-center gap-4 text-xs text-stone-500 mb-4">
                ...
                <span className="flex items-center gap-1">
                  <span className="material-symbols-outlined text-sm">calendar_today</span>Since{' '}
                  {memberSince}
                </span>
              </div>
              <div className="flex gap-2 justify-center">
                ...
                {isOwnProfile && (
                  <Button
                    onClick={() => router.push('/settings')}
                    className="bg-[var(--deep-teal)] text-white px-5 py-2 rounded-full font-bold hover:bg-[var(--primary-color)] transition shadow-md flex items-center gap-1.5 text-sm"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            {/* stats */}
            <div
              className={`bg-white rounded-xl shadow-sm border border-orange-100 p-6 grid gap-4 text-center ${isArtLoverProfile ? 'grid-cols-1' : 'grid-cols-3'}`}
            >
              <div>
                <span className="text-2xl font-bold text-[var(--primary-color)] block">
                  {u.followerCount ?? u.followersCount ?? u.stats?.followers ?? '0'}
                </span>
              ...
              {!isArtLoverProfile && (
                <div>
                  <span className="text-2xl font-bold text-[var(--primary-color)] block">
                    {u.performanceCount ?? u.performancesCount ?? u.stats?.performances ?? '0'}
                  </span>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-orange-100 p-6">
              <h3 className="font-bold text-lg text-[var(--text-primary)] mb-3 serif-font">
                About
              </h3>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-orange-100 p-6">
                ...
                <div className="space-y-3">
                  ...
                  {socialLinks.length === 0 && (
                    <p className="text-sm text-stone-400">No social links added.</p>
                  )}
                </div>
              </div>
            )}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-orange-100 p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {(u.specializations || []).map((tag) => (
                    <span
                      key={tag}
                      className="bg-orange-50 text-[var(--primary-color)] px-3 py-1 rounded-full text-sm border border-orange-200"
                    >
                      {tag}
                    </span>
                  ))}
                  {(!u.specializations || u.specializations.length === 0) && (
                    <p className="text-sm text-stone-400">No specializations listed.</p>
                  )}
                </div>
              </div>
            )}
                  ...
                  {(u.pricing || []).map((p, i) => (
                    <div key={i} className="flex justify-between">
                      ...
                      <span className="font-bold text-[var(--text-primary)]">
                        ₹ {p.amount?.toLocaleString() || p.price?.toLocaleString()}
                      </span>
                    </div>
                  ))}
                  {(!u.pricing || u.pricing.length === 0) && (
                    <p className="text-stone-400">Pricing not listed.</p>
                  )}
                </div>
                <Button
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
                <h2 className="text-2xl font-bold text-[var(--text-primary)] font-display">
                  {isArtLoverProfile ? 'Posts' : 'Portfolio'}
                </h2>
                {!isArtLoverProfile && (
              ...
              {portfolio.length === 0 && (
                <EmptyState
                  ...
                  description={isArtLoverProfile ? 'No posts yet.' : 'No portfolio items yet.'}
                />
              )}
              <div className="space-y-4">
                {portfolio
                  ...
                  .map((post, i) => (
                    <article
                      key={post._id || i}
                      className="bg-[var(--surface-color)] rounded-xl shadow-sm border border-orange-100 overflow-hidden"
                    >
                      {/* header */}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-stone-500">
                              <span className="capitalize">{post.postType || 'general'}</span> •{' '}
                          ...
                          {openMenuId === post._id && (
                            <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-xl shadow-xl border border-stone-200 z-20 overflow-hidden text-sm">
                              <Button
                                ...
                                className="w-full flex items-center gap-2 px-4 py-2.5 hover:bg-stone-50 text-stone-700"
                              >
                              ...
                              {isOwnProfile && (
                                <Button
                                  ...
                                  className="w-full flex items-center gap-2 px-4 py-2.5 hover:bg-red-50 text-red-500 border-t border-stone-100"
                                >
                                  <span className="material-symbols-outlined text-base">
                                    delete
                                  </span>
                            ...
                            {!single && (
                              <>
                                <Button
                                  ...
                                  className="absolute left-6 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center"
                                >
                                  <span className="material-symbols-outlined text-lg">
                                    chevron_left
                                  </span>
                                ...
                                <Button
                                  ...
                                  className="absolute right-6 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center"
                                >
                                  <span className="material-symbols-outlined text-lg">
                                    chevron_right
                                  </span>
                                ...
                                <div className="flex justify-center gap-1.5 mt-2">
                                  {images.map((_, di) => (
                                    <Button
                                      key={di}
                                      onClick={() =>
                                        setCarouselIndex((prev) => ({ ...prev, [post._id]: di }))
                                      }
                        ...
                        <div className="mx-4 mb-4 bg-orange-50 border border-orange-100 rounded-lg p-4 flex gap-4 items-center">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-[var(--text-secondary)] mt-1">
                              ...
                              {post.embeddedEvent.location}
                            </p>
                            <p className="text-xs text-[var(--text-secondary)] mt-0.5">
                              <span className="material-symbols-outlined align-middle text-sm">
                                schedule
                              </span>{' '}
                              {post.embeddedEvent.time}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="p-4 flex flex-wrap items-center justify-between gap-3 border-t border-stone-100">
                        <div className="flex items-center gap-4">
                          <Button
                            ...
                            className={`flex items-center gap-1.5 transition-colors ${post.isLiked ? 'text-red-500' : 'text-stone-600 hover:text-red-500'}`}
                          >
                            <span
                              className={`material-symbols-outlined${post.isLiked ? ' filled' : ''}`}
                            >
                              favorite
                            </span>
                          ...
                          <Button
                            ...
                            className={`flex items-center gap-1.5 transition-colors ${expandedComments.has(post._id) ? 'text-[var(--secondary-color)]' : 'text-stone-600 hover:text-[var(--secondary-color)]'}`}
                          >
                            <span className="material-symbols-outlined">chat_bubble</span>
                            <span className="text-sm font-medium">
                              {commentsMap[post._id]?.length ?? post.commentCount ?? 0}
                            </span>
                          </Button>
                          <Button
                            onClick={() => sharePost(post._id)}
                            className={`flex items-center gap-1.5 transition-colors ${sharedPosts.has(post._id) ? 'text-green-600' : 'text-stone-600 hover:text-[var(--secondary-color)]'}`}
                          >
                        ...
                        <div className="px-4 pb-4 space-y-3 border-t border-stone-100 pt-3 bg-stone-50">
                          {(commentsMap[post._id] || []).map((c, ci) => (
                            <div key={c._id || ci} className="flex gap-2 items-start">
                              <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0 bg-stone-200">
                                <Image
                                  src={c.author?.avatar || '/avatar-placeholder.svg'}
                                  alt=""
                                  className="w-full h-full object-cover"
                                  width={28}
                                  height={28}
                              ...
                              <div className="bg-white rounded-xl px-3 py-2 text-xs flex-1 border border-stone-100">
                                <span className="font-bold text-[var(--text-primary)] mr-1">
                          ...
                          <div className="flex gap-2 items-center">
                            <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0">
                              <Image
                                src={authUser?.avatar || '/avatar-placeholder.svg'}
                                alt=""
                                className="w-full h-full object-cover"
                                width={28}
                                height={28}
                            ...
                            <Button
                              ...
                              disabled={
                                submittingComment[post._id] || !commentInputs[post._id]?.trim()
                              }
            ...
            {!isArtLoverProfile && (
              <div>
                <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-4 font-display">
                  Reviews
                </h2>
                ...
                <div className="space-y-4">
                  {reviews.map((review, i) => (
                    <div
                      key={review._id || i}
                      className="bg-white border border-orange-100 rounded-xl p-6 shadow-sm"
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <h4 className="font-bold text-[var(--text-primary)]">
                            {review.name || review.reviewer?.fullName || 'Reviewer'}
                          </h4>
