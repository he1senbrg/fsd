        ...
        <AppShell>
            <div className="py-2">
                ...
                <nav className="text-sm text-stone-500 mb-6 flex items-center gap-1">
                    <Link href="/marketplace" className="hover:text-[var(--primary-color)] transition-colors">Marketplace</Link>
                ...
                <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-8 font-display flex items-center gap-3">
                    <span className="material-symbols-outlined text-[var(--primary-color)]">shopping_cart</span>
                    My Cart
                    {items.length > 0 && (
                        <span className="text-base font-normal text-stone-500">({items.length} item{items.length > 1 ? "s" : ""})</span>
                    )}
                </h1>
                ...
                {items.length === 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        <div className="w-24 h-24 rounded-full bg-orange-50 flex items-center justify-center mb-6">
                            <span className="material-symbols-outlined text-5xl text-[var(--primary-color)]/40">shopping_cart</span>
                        </div>
                        <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-2 font-display">Your cart is empty</h2>
                        <p className="text-stone-500 mb-8 max-w-sm">Discover handcrafted treasures from talented artisans across India.</p>
                        <Link href="/marketplace" className="btn-primary px-8 py-3 text-base shadow-md flex items-center gap-2">
                    ...
                    <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="flex-1 space-y-4">
                            {items.map((item) => (
                                <div key={item._id} className="bg-white rounded-xl shadow-sm border border-orange-100 p-4 flex gap-4 group">
                                    {/* pdt img */}
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-stone-500 mb-2">
                                                    {item.product?.category && (
                                                        <span className="uppercase tracking-wider font-semibold text-[var(--secondary-color)]">{item.product.category}</span>
                                                    )}
                                                    {item.product?.seller?.fullName && ` · by ${item.product.seller.fullName}`}
                                                </p>
                                                <p className="text-lg font-bold text-[var(--text-primary)]">
                                                    ₹ {((item.product?.price || 0) * (item.quantity || 1)).toLocaleString()}
                                                </p>
                                                {item.quantity > 1 && (
                                                    <p className="text-xs text-stone-400">₹ {(item.product?.price || 0).toLocaleString()} × {item.quantity}</p>
                                                )}
                                            </div>
                                            {/* stock warning */}
                            ...
                            {/* continue shopping */}
                            <Link href="/marketplace" className="flex items-center gap-2 text-sm text-[var(--primary-color)] font-medium hover:underline mt-2">
                        ...
                        <div className="lg:w-80 flex-shrink-0 space-y-4">
                            <div className="bg-white rounded-xl shadow-sm border border-orange-100 p-6">
                                <h2 className="font-bold text-lg text-[var(--text-primary)] font-display mb-5">Order Summary</h2>
                                <div className="space-y-3 text-sm text-stone-600">
                                    <div className="flex justify-between">
                                        <span>Subtotal ({items.length} item{items.length > 1 ? "s" : ""})</span>
                                        <span className="font-medium text-[var(--text-primary)]">₹ {subtotal.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium text-[var(--text-primary)]">₹ {platformFee.toLocaleString()}</span>
                                    </div>
                                ...
                                <Button
                                    onClick={() => router.push(`/payment?type=cart&amount=${total}&title=${encodeURIComponent("Cart Checkout")}`)}
                                ...
                                <div className="flex items-center justify-center gap-4 mt-4 text-xs text-stone-400">
                                    <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm text-green-500">verified_user</span>Secure</span>
                                    <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm text-blue-500">payments</span>Encrypted</span>
                                    <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm text-orange-400">cached</span>Easy Returns</span>
                                </div>
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-4 border border-orange-100">
                                <p className="text-xs text-[var(--text-secondary)] font-medium mb-1 flex items-center gap-1">
                                    <span className="material-symbols-outlined text-sm text-[var(--primary-color)]">volunteer_activism</span>
                                    Support Artisans
                                </p>
                                <p className="text-xs text-stone-500">Every purchase directly supports a skilled artisan and helps preserve India&apos;s cultural heritage.</p>
                            </div>
                        </div>
                    </div>
                )}
