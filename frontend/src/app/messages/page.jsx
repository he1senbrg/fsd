...
function MessagesContent() {
    ...
    useEffect(() => {
        ...
        sock.on('message:new', (msg) => {
            if (msg.conversation === currentConvIdRef.current || msg.conversation?._id === currentConvIdRef.current) {
    ...
    useEffect(() => {
        ...
        conversationAPI.getMessages(conv._id)
    ...
    const getOtherParticipant = (conv) => {
        if (!conv?.participants || !authUser) return { name: "Unknown", avatar: "/avatar-placeholder.svg", role: "" };
        const other = conv.participants.find(p => (p._id || p) !== authUser._id) || conv.participants[0];
        return {
    ...
    const currentOther = currentConv ? getOtherParticipant(currentConv) : { name: "Select a conversation", avatar: "/avatar-placeholder.svg" };
        ...
        <>
            ...
            <div className="max-w-7xl mx-auto px-4 py-4 h-[calc(100vh-4rem)]">
                ...
                <div className="flex gap-4 h-[calc(100%-3rem)] md:h-full">
                    {/* sidebar */}
                    <div className={`${activeView === "list" ? "flex" : "hidden"} md:flex w-full md:w-80 flex-shrink-0 flex-col bg-white rounded-xl shadow-sm border border-orange-100 overflow-hidden`}>
                        <div className="p-4 border-b border-stone-100">
                            ...
                            <div className="relative">
                                ...
                                <input className="w-full bg-stone-50 border-none rounded-lg py-2 pl-9 pr-4 text-sm focus:ring-2 focus:ring-[var(--secondary-color)]" placeholder="Search messages..." type="text" />
                            </div>
                        </div>
                        <div className="flex-1 overflow-y-auto">
                            {conversations.length === 0 && (
                                <EmptyState className="py-8" icon="chat_bubble" iconClassName="text-3xl" description="No conversations yet." />
                            )}
                                ...
                                return (
                                    ...
                                    >
                                        <div className="relative flex-shrink-0">
                                            <Image alt={other.name} className="w-12 h-12 rounded-full object-cover" src={other.avatar} width={48} height={48} unoptimized />
                                            <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 rounded-full border-2 border-white"></span>
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline">
                                                <h4 className="font-semibold text-sm text-[var(--text-primary)] truncate">{other.name}</h4>
                                                <span className="text-[10px] text-stone-400 flex-shrink-0">{timeAgo(conv.lastMessageAt || conv.updatedAt)}</span>
                                            </div>
                                            <p className="text-xs text-stone-500 truncate">{conv.lastMessage?.text || conv.lastMsg || ""}</p>
                                        </div>
                                        {(conv.unreadCount || 0) > 0 && (
                                            <span className="bg-[var(--primary-color)] text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center font-bold flex-shrink-0">{conv.unreadCount}</span>
                                        )}
                                    </div>
                                );
                            })}
                    ...
                    {/* chat screen */}
                    <div className={`${activeView === "chat" ? "flex" : "hidden"} md:flex flex-1 flex-col bg-white rounded-xl shadow-sm border border-orange-100 overflow-hidden`}>
                        <div className="p-3 sm:p-4 border-b border-stone-100 flex items-center gap-3">
                            <Button onClick={() => setActiveView("list")} className="md:hidden text-stone-400 hover:text-[var(--primary-color)] -ml-1">
                                <span className="material-symbols-outlined">arrow_back</span>
                            </Button>
                            <Image alt="Chat" className="w-10 h-10 rounded-full object-cover" src={currentOther.avatar} width={40} height={40} unoptimized />
                            <div className="flex-1 min-w-0">
                            ...
                            {chatMessages.map((msg, i) => {
                                ...
                                return (
                                    <div key={msg._id || i} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                                        <div className={`max-w-[85%] sm:max-w-[70%] px-3 sm:px-4 py-2.5 rounded-2xl text-sm ${isMe ? "bg-[var(--primary-color)] text-white rounded-br-sm" : "bg-white text-[var(--text-primary)] rounded-bl-sm shadow-sm border border-stone-100"}`}>
                                            <p>{msg.text || msg.content}</p>
                                            <span className={`text-[10px] mt-1 block text-right ${isMe ? "text-white/60" : "text-stone-400"}`}>
                        ...
                        <div className="p-3 border-t border-stone-100 flex items-center gap-2 sm:gap-3">
                            ...
                            />
                            <Button onClick={sendMessage} className="bg-[var(--primary-color)] text-white w-10 h-10 rounded-full hover:bg-[var(--secondary-color)] transition-colors flex-shrink-0 flex items-center justify-center"><span className="material-symbols-outlined text-lg">send</span></Button>
                        </div>
                    </div>
                </div>
            </div>
