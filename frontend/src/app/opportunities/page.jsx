...
export default function OpportunitiesPage() {
    ...
    const submitApplication = async (opp) => {
        ...
        try {
            ...
            setApplyActive(prev => { const next = new Set(prev); next.delete(opp._id); return next; });
    ...
    const handleEventAction = async (event) => {
        if (!event._id) return;
        const isFree = event.price === 0 || event.eventType === "free" || event.price === "Free Entry" || !event.price;
        if (!isFree) {
            // redirect to payment page if paid event
            const amount = typeof event.price === "number" ? event.price : (event.ticketTiers?.[0]?.price || 0);
            const tier = event.ticketTiers?.[0]?.name || "";
            router.push(
                `/payment?type=event&id=${event._id}&title=${encodeURIComponent(event.title || "Event Ticket")}&amount=${amount}&tier=${encodeURIComponent(tier)}`
            );
            return;
        }
        ...
        <AppShell>
            <div className="py-2">
                ...
                <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-6 font-display flex items-center gap-2">
                    <span className="material-symbols-outlined text-[var(--terracotta)]">event</span>Upcoming Events
                </h2>
                {events.length === 0 && (
                    <EmptyState className="py-8 mb-8" icon="event_busy" iconClassName="text-4xl" description="No upcoming events." />
                )}
                        ...
                        return (
                            <div key={event._id || i} className="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col md:flex-row h-auto md:h-52">
                                ...
                                <div className="md:w-2/3 p-6 flex flex-col justify-between">
                                    <div>
                                        <span className="text-[var(--terracotta)] text-xs font-bold uppercase tracking-wider mb-1 block">{event.category}</span>
                                        <h3 className="text-xl font-bold text-[var(--text-primary)] font-display group-hover:text-[var(--terracotta)] transition-colors">{event.title}</h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-12">
                    {openCalls.filter(call => {
                        if (activeTab !== "All" && activeTab !== "Open Calls" && activeTab !== "Gigs") return false;
                        ...
                        <div key={call._id || i} className="bg-white rounded-xl shadow-sm border border-orange-100 p-6 hover:-translate-y-1 transition-transform group">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div className="flex-1">
                                    <h3 className="font-bold text-[var(--text-primary)] group-hover:text-[var(--terracotta)] transition-colors text-sm">{call.title}</h3>
                                    <p className="text-xs text-stone-500">{call.org || call.organization}</p>
                                </div>
                            </div>
                            <div className="text-xs text-stone-500 space-y-1 mb-3">
                                <p><span className="material-symbols-outlined text-sm align-middle">location_on</span> {call.location}</p>
                                <p><span className="material-symbols-outlined text-sm align-middle">schedule</span> Deadline: {call.deadline ? new Date(call.deadline).toLocaleDateString("en-IN", { month: "short", day: "numeric" }) : "Open"}</p>
                                <p className="font-bold text-[var(--text-primary)]"><span className="material-symbols-outlined text-sm align-middle">payments</span> {call.pay || call.compensation || "Contact"}</p>
                            </div>
                            <div className="flex gap-2 mb-4 flex-wrap">
                                {(call.tags || call.categories || []).map(tag => (
                                    <span key={tag} className="bg-orange-50 text-[var(--primary-color)] text-[10px] px-2 py-0.5 rounded-full border border-orange-200">{tag}</span>
                                ))}
                            </div>
                                ...
                                {applyActive.has(call._id) && (
                                    <div className="flex flex-col gap-2">
                                        <textarea
                                            ...
                                            onChange={(e) => setApplyTexts(prev => ({ ...prev, [call._id]: e.target.value }))}
                                        />
                                        <Button
                                            variant="success"
                                        ...
                                        >{applyLoading[call._id] ? 'Submitting...' : 'Submit Application'}</Button>
                                    </div>
                                )}
                            </div>
                        </div>
