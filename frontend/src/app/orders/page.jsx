...
export default function OrdersPage() {
    ...
    const contactSeller = async (order) => {
        ...
        if (!sellerId) { showToast("Seller contact not available.", "warning"); return; }
    ...
    return (
        <AppShell>
            <div className="py-2">
                ...
                {successBanner && (
                    <div className="mb-6 bg-green-50 border border-green-200 rounded-2xl p-5 flex items-start gap-4 shadow-sm">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-green-700">Your order/booking has been confirmed. You can track it below.</p>
                        </div>
                        <Button onClick={() => setSuccessBanner(false)} className="text-green-400 hover:text-green-600 transition-colors mt-0.5">
                ...
                {/* orders */}
                {filtered.length === 0 && (
                    <EmptyState description="No orders found." icon="receipt_long" />
                )}
                <div className="space-y-4">
                    {filtered.map((order) => {
                        const statusColor = statusColorMap[order.status?.toLowerCase()] || "text-stone-600 bg-stone-50";
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-start justify-between mb-1.5 gap-2">
                                        <div className="min-w-0">
                                            <h3 className="font-bold text-[var(--text-primary)] text-base leading-snug truncate">{displayTitle}</h3>
                                            {sellerName && (
                                        ...
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap flex-shrink-0 ${statusColor}`}>
                                            {order.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-stone-500 mt-2">
                                        <span className="flex items-center gap-1">
                                            ...
                                            <span className="font-mono text-xs text-stone-600">{order.orderId || order._id}</span>
                                        </span>
                                        <span className="flex items-center gap-1">
                                    ...
                                    {isBooking && order.event?.startDate && (
                                        <p className="text-xs text-stone-500 mt-1.5 flex items-center gap-1">
                                            ...
                                            {new Date(order.event.startDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                                    ...
                                    <div className="flex flex-col gap-3 mt-3">
                                        ...
                                        {trackingData[order._id] && (
                                            <div className="bg-stone-50 rounded-lg p-3 text-xs border border-stone-200 space-y-1">
                                                ...
                                                <p>Carrier: <strong>{trackingData[order._id]?.carrier || "N/A"}</strong></p>
                                                <p>Tracking #: <strong className="font-mono">{trackingData[order._id]?.trackingNumber || "N/A"}</strong></p>
                                                <p>Status: <strong>{trackingData[order._id]?.status || "In Transit"}</strong></p>
                                                {trackingData[order._id]?.estimatedDelivery && (
                                                    <p>Est. Delivery: <strong>{new Date(trackingData[order._id].estimatedDelivery).toLocaleDateString("en-IN")}</strong></p>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {ticketData[order._id] && (
                                            <div className="bg-orange-50 rounded-lg p-3 text-xs border border-orange-200 space-y-1">
                                                ...
                                                {ticketData[order._id]?.ticket?.event?.venue && (
                                                    <p>Venue: <strong>{ticketData[order._id].ticket.event.venue}</strong></p>
                                                )}
                                            </div>
                                        )}
