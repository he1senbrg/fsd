            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <main className="lg:col-span-2 space-y-6">
                    ...
                    {!postsLoading && posts.map(post => (
                        <article id={`post-${post._id}`} key={post._id} className="bg-[var(--surface-color)] rounded-xl shadow-sm border border-orange-100 overflow-hidden">
                            ...
                            <div className="p-4 flex flex-wrap items-center justify-between gap-3 border-t border-stone-100">
                                <div className="flex items-center gap-4 sm:gap-6">
                                    ...
                                    <Button onClick={() => sharePost(post._id)} className={`flex items-center gap-1.5 sm:gap-2 transition-colors ${sharedPosts.has(post._id) ? "text-green-600" : "text-stone-600 hover:text-[var(--secondary-color)]"}`}><span className="material-symbols-outlined">share</span><span className="text-sm font-medium">{(post.shareCount ?? 0).toLocaleString()}</span>{sharedPosts.has(post._id) && <span className="text-xs">Shared!</span>}</Button>
                                </div>
                            </div>
                            {/* comments */}
