...
export default function FeedClient() {
  ...
  useEffect(() => {
    ...
    const targetExists = posts.some((p) => String(p._id) === String(targetPostId));
  ...
  const toggleLike = async (id) => {
    try {
      ...
      setPosts((prev) =>
        prev.map((p) =>
          p._id === id ? { ...p, likeCount: res.data.likeCount, isLiked: res.data.isLiked } : p,
        ),
      );
    } catch (e) {
      if (e?.status !== 401) console.error(e);
            ...
            <div className="flex flex-wrap items-center justify-between border-t border-stone-100 pt-3 gap-3">
              <div className="flex gap-3 sm:gap-4">
                <Button
                  ...
                  className="flex items-center gap-1 sm:gap-2 text-stone-500 hover:text-[var(--secondary-color)] text-sm"
                >
                  <span className="material-symbols-outlined text-xl">image</span> Photo
                </Button>
                <Button
                  onClick={() => videoInputRef.current?.click()}
                  className="flex items-center gap-1 sm:gap-2 text-stone-500 hover:text-[var(--secondary-color)] text-sm"
                >
                  <span className="material-symbols-outlined text-xl">movie</span> Video
                </Button>
              </div>
              <Button
                ...
                disabled={
                  posting ||
                  (!postContent.trim() && selectedFiles.length === 0 && !selectedVideoFile)
                }
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden cursor-pointer">
                      <Image
                        alt="Artist"
                        className="w-full h-full object-cover"
                        src={post.author?.avatar || '/avatar-placeholder.svg'}
                        width={40}
                        height={40}
                        unoptimized
                      />
                    ...
                    <div>
                      <h4 className="font-bold text-[var(--text-primary)] text-sm cursor-pointer hover:underline">
                        {post.author?.fullName || 'Unknown'}
                      </h4>
                      <p className="text-xs text-stone-500">
                        {post.author?.title || post.author?.role || ''} • {timeAgo(post.createdAt)}
                      </p>
                    </div>
                  </div>
                ...
                <div className="px-4 pb-3">
                  <p className="text-[var(--text-secondary)] text-sm mb-2">
                    ...
                    {post.hashtags && (
                      <span className="text-[var(--secondary-color)]">{post.hashtags}</span>
                    )}
                  </p>
                </div>
                          ...
                          <div className="flex justify-center gap-1.5 mt-2">
                            {images.map((_, i) => (
                              <Button
                                key={i}
                                onClick={() =>
                                  setCarouselIndex((prev) => ({ ...prev, [post._id]: i }))
                                }
                  ...
                  <div className="mx-4 mb-4 bg-orange-50 border border-orange-100 rounded-lg p-4 flex gap-4 items-center">
                    ...
                    <Button
                      onClick={() =>
                        router.push(
                          post.event._id
                            ? `/opportunities?event=${post.event._id}`
                            : '/opportunities',
                        )
                      }
                      className="bg-white border border-[var(--primary-color)] text-[var(--primary-color)] px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-[var(--primary-color)] hover:text-white transition-colors"
                    >
                      Details
                    </Button>
                  </div>
                ...
                <div className="p-4 flex flex-wrap items-center justify-between gap-3 border-t border-stone-100">
                  <div className="flex items-center gap-4 sm:gap-6">
                    <Button
                      ...
                      className={`flex items-center gap-1.5 sm:gap-2 transition-colors ${post.isLiked ? 'text-red-500' : 'text-stone-600 hover:text-red-500'}`}
                    >
                      <span className={`material-symbols-outlined${post.isLiked ? ' filled' : ''}`}>
                        favorite
                      </span>
                    ...
                    <Button
                      ...
                      className={`flex items-center gap-1.5 sm:gap-2 transition-colors ${expandedComments.has(post._id) ? 'text-[var(--secondary-color)]' : 'text-stone-600 hover:text-[var(--secondary-color)]'}`}
                    >
                    ...
                    <Button
                      ...
                      className={`flex items-center gap-1.5 sm:gap-2 transition-colors ${sharedPosts.has(post._id) ? 'text-green-600' : 'text-stone-600 hover:text-[var(--secondary-color)]'}`}
                    >
        ...
        <aside className="hidden lg:block space-y-6">
          <div className="bg-[var(--surface-color)] rounded-xl shadow-sm border border-orange-100 p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-[var(--text-primary)] text-md serif-font">
                Trending Opportunities
              </h3>
              <a
                className="text-xs text-[var(--secondary-color)] font-semibold hover:underline"
                href="/opportunities"
              >
                View All
              </a>
            ...
            <div className="space-y-4">
              {opportunities.map((opp, i) => (
                ...
                >
                  <p
                    className={`text-xs font-bold mb-1 uppercase tracking-wide ${opp.type === 'Event' ? 'text-blue-600' : opp.type === 'Exhibition' ? 'text-purple-600' : 'text-green-600'}`}
                  >
                    {opp.type || 'Open Call'}
                  </p>
                  <h4 className="font-semibold text-sm text-[var(--text-primary)] hover:text-[var(--secondary-color)] cursor-pointer">
                    {opp.title}
                  </h4>
                  <p className="text-xs text-stone-500 mt-1">
                    {opp.organization || opp.org} •{' '}
                    {opp.deadline
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <span className="bg-stone-100 text-stone-600 text-[10px] px-2 py-0.5 rounded-full">
                      {opp.compensation || opp.tag || 'Open'}
                    </span>
                    <Button
                      onClick={() => applyToOpportunity(opp)}
                      className="text-xs text-[var(--primary-color)] font-medium hover:underline"
                    >
                      Apply
                    </Button>
                  </div>
              ...
              {opportunities.length === 0 && (
                <p className="text-xs text-stone-400 text-center py-2">No opportunities yet</p>
              )}
            </div>
          </div>
            ...
            <div className="space-y-4">
              {campaigns.map((c, i) => {
                const percent = c.goalAmount
                  ? Math.round(((c.raisedAmount || c.currentAmount || 0) / c.goalAmount) * 100)
                  : 0;
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex justify-between mt-1">
                        <span className="text-[10px] text-stone-500">
                          ₹{(raised / 1000).toFixed(0)}k raised
                        </span>
              ...
              {campaigns.length === 0 && (
                <p className="text-xs text-stone-400 text-center py-2">No campaigns yet</p>
              )}
            </div>
            <Button
              onClick={() => router.push('/crowdfunding/create')}
              className="w-full mt-4 border border-[var(--primary-color)] text-[var(--primary-color)] text-xs font-semibold py-2 rounded-lg hover:bg-[var(--primary-color)] hover:text-white transition-colors"
            >
              Start a Campaign
            </Button>
          </div>
