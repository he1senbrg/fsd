    ...
    return (
        <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                    <span className={`material-symbols-outlined ${(hovered || value) >= star ? "text-yellow-400 filled" : "text-stone-300"} text-xl`}>
                        star
                    </span>
                </button>
            ))}
            ...
            <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-full bg-[var(--primary-color)] flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {review.reviewer?.avatar ? (
                        ...
                        <span className="text-white font-bold text-sm">{review.reviewer?.fullName?.[0] || "?"}</span>
                    )}
                </div>
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                        <p className="font-semibold text-sm text-[var(--text-primary)]">{review.reviewer?.fullName || "Anonymous"}</p>
                        <span className="text-xs text-stone-400">{dateStr}</span>
                    </div>
                    <StarRating value={review.rating} readonly />
                    {review.text && <p className="text-sm text-stone-600 mt-1 leading-relaxed">{review.text}</p>}
                </div>
            </div>
        </div>
    );
        ...
        return (
            <AppShell>
                <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
                    ...
                    <h2 className="text-2xl font-bold text-[var(--text-primary)] font-display">Product Not Found</h2>
                    <Button onClick={() => router.push("/marketplace")} className="bg-[var(--primary-color)] text-white px-6 py-2 rounded-full font-bold">Back to Marketplace</Button>
                </div>
            </AppShell>
        );
    }
        ...
        <AppShell>
            <div className="py-2 max-w-6xl mx-auto">
                ...
                <nav className="text-sm text-stone-500 mb-6 flex items-center gap-1 flex-wrap">
                    <Link className="hover:text-[var(--primary-color)]" href="/marketplace">Marketplace</Link>
                    <span>/</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-14">
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-start justify-between gap-2 mb-2">
                            <div>
                                <p className="text-xs font-bold text-[var(--secondary-color)] uppercase tracking-wider mb-1">{product.category}</p>
                                <h1 className="font-display text-3xl font-bold text-[var(--text-primary)] leading-tight">{product.name}</h1>
                            </div>
                        </div>

                        {/* seller */}
                        <p className="text-sm text-stone-500 mb-3">
                            By <span className="text-stone-800 font-semibold">{product.seller?.fullName || "Artisan"}</span>
                        ...
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            {reviewCount > 0 && <span className="text-sm text-stone-400">({reviewCount} {reviewCount === 1 ? "review" : "reviews"})</span>}
                        </div>
                        ...
                        {product.stock > 0 ? (
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div className="flex items-center border border-stone-200 rounded-lg overflow-hidden">
                                    <button onClick={() => setQty(q => Math.max(1, q - 1))} className="px-3 py-2 text-stone-500 hover:bg-stone-50 transition-colors text-lg leading-none">−</button>
                                    <span className="px-4 py-2 font-semibold text-sm min-w-[3rem] text-center">{qty}</span>
                                    <button onClick={() => setQty(q => Math.min(product.stock, q + 1))} className="px-3 py-2 text-stone-500 hover:bg-stone-50 transition-colors text-lg leading-none">+</button>
                                </div>
                                <span className="text-xs text-stone-400">{product.stock} in stock</span>
                            </div>
                        ) : (
                            <div className="mb-6">
                                <span className="inline-block bg-red-50 text-red-600 text-sm font-semibold px-3 py-1 rounded-full border border-red-100">Out of Stock</span>
                            </div>
                        )}
                    ...
                    <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6 mb-10">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            {[
                                ...
                                { label: "Badge", key: "badge", type: "text", placeholder: "e.g. New Arrival, Bestseller" },
                            ].map(({ label, key, type, placeholder }) => (
                                <div key={key}>
                                    <label className="text-xs font-semibold text-stone-500 mb-1 block uppercase tracking-wide">{label}</label>
                                    <input
                                        type={type}
                                        value={editForm[key] ?? ""}
                        ...
                        <div className="mb-4">
                            ...
                            >
                                {["textiles", "woodwork", "jewelry", "pottery", "metalCrafts", "paintings"].map(c => (
                        ...
                        <div className="mb-6">
                            <label className="text-xs font-semibold text-stone-500 mb-2 block uppercase tracking-wide">Replace Images</label>
                            <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handleEditImageChange} />
                        ...
                        <div className="flex gap-3 justify-end">
                            <Button onClick={() => setShowEdit(false)} className="px-5 py-2 rounded-lg border border-stone-300 text-stone-600 text-sm hover:bg-stone-50 transition-colors">Cancel</Button>
                            <Button
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="bg-[var(--primary-light,#fef3ec)] rounded-2xl p-6 text-center mb-6">
                            <p className="text-6xl font-bold text-[var(--primary-color)]">{avgRating > 0 ? avgRating.toFixed(1) : "—"}</p>
                            <StarRating value={Math.round(avgRating)} readonly />
                            <p className="text-stone-500 text-sm mt-1">{reviewCount} {reviewCount === 1 ? "review" : "reviews"}</p>
                        </div>
                        ...
                        {!user && (
                            <p className="text-sm text-stone-500 text-center">
                                <Link href="/login" className="text-[var(--primary-color)] font-semibold hover:underline">Sign in</Link> to leave a review.
                            </p>
                        )}
                    </div>
                    ...
                    <div className="lg:col-span-3">
                        {reviews.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-16 text-center gap-3 bg-stone-50 rounded-2xl">
                                <span className="material-symbols-outlined text-4xl text-stone-300">rate_review</span>
