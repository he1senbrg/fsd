...
const REGIONS = ["Rajasthan", "Kerala", "West Bengal", "Gujarat", "Maharashtra", "Tamil Nadu", "Uttar Pradesh", "Odisha", "Other"];
...
export default function CreateProductPage() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.name.trim()) { showToast("Product name is required.", "warning"); return; }
        if (!form.price || Number(form.price) <= 0) { showToast("Please enter a valid price.", "warning"); return; }
        if (!form.stock || Number(form.stock) < 0) { showToast("Stock must be 0 or more.", "warning"); return; }
        if (imageFiles.length === 0) { showToast("Please upload at least one product image.", "warning"); return; }
    ...
    if (authLoading) return <AppShell><Loader /></AppShell>;
        ...
        return (
            <AppShell>
                <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
                    ...
                    <h2 className="text-2xl font-bold text-[var(--text-primary)] font-display">Artists Only</h2>
                    <p className="text-[var(--text-secondary)] max-w-sm">
                        Only registered artists can list products on the marketplace. Update your role in Settings.
                    </p>
                    <Button onClick={() => router.push("/marketplace")} className="mt-2 bg-[var(--primary-color)] text-white px-6 py-2 rounded-full font-bold">
                        Back to Marketplace
                    </Button>
                </div>
            </AppShell>
    ...
    return (
        <AppShell>
            <div className="py-2 max-w-3xl mx-auto">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-[var(--text-primary)] font-display mb-2">List a Product</h1>
                    <p className="text-[var(--text-secondary)]">Showcase your craft to buyers across India. Fill in the details below to list your product.</p>
                </div>
                ...
                <SurfaceCard className="p-4 sm:p-6 md:p-8">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {/* name */}
                        <div>
                            <label className="text-sm font-semibold text-[var(--text-primary)] mb-1 block">Product Name *</label>
                            <FormInput
                                value={form.name}
                                onChange={set("name")}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="text-sm font-semibold text-[var(--text-primary)] mb-1 block">Region</label>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label className="text-sm font-semibold text-[var(--text-primary)] mb-1 block">Price (₹) *</label>
                                <FormInput
                                    value={form.price}
                                    onChange={set("price")}
                            ...
                            <div>
                                <label className="text-sm font-semibold text-[var(--text-primary)] mb-1 block">Stock Qty *</label>
                                <FormInput
                                    value={form.stock}
                                    onChange={set("stock")}
                                    placeholder="e.g., 10"
                        ...
                        {form.name && form.price && (
                            <div className="bg-orange-50 rounded-xl p-4 border border-orange-100 flex items-center gap-4 flex-wrap">
                                <span className="material-symbols-outlined text-[var(--primary-color)]">inventory_2</span>
                                <div className="text-sm text-stone-700">
                                    <strong>{form.name}</strong>
                                    {form.category && <span className="text-stone-400 ml-2">· {form.category}</span>}
                                    {form.price && <span className="font-bold text-[var(--primary-color)] ml-2">₹{Number(form.price).toLocaleString("en-IN")}</span>}
                                    {form.stock && <span className="text-stone-400 ml-2">· {form.stock} in stock</span>}
                                </div>
                            </div>
                        )}
