        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            <Link href="/" className="flex-shrink-0 flex items-center gap-2 cursor-pointer">
              ...
              <span className="hidden sm:block text-3xl font-bold text-[var(--deep-teal)] tracking-tight font-display">
                KalaSetu
              </span>
            </Link>
            <div className="hidden md:flex space-x-8 items-center">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-[var(--deep-teal)] hover:text-[var(--terracotta)] font-medium transition-colors"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="hidden md:flex items-center space-x-4">
              <Link
                href="/login"
                className="text-[var(--deep-teal)] font-bold hover:text-[var(--terracotta)] transition-colors"
              >
                Log In
              </Link>
              <Link
                href="/login"
                className="bg-[var(--deep-teal)] text-[var(--cream)] px-5 py-2 rounded-full font-bold hover:bg-[var(--terracotta)] transition-colors shadow-md"
              >
                Sign Up
              </Link>
            </div>
            <div className="md:hidden flex items-center">
              <Button
                ...
                className="text-[var(--deep-teal)] hover:text-[var(--terracotta)] focus:outline-none"
              >
            ...
            <div className="md:hidden pb-4 space-y-2">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="block py-2 text-[var(--deep-teal)] hover:text-[var(--terracotta)] font-medium"
                >
                  {link.label}
                </Link>
              ))}
              <Link href="/login" className="block py-2 text-[var(--deep-teal)] font-bold">
                Log In / Sign Up
              </Link>
            </div>
          )}
    ...
    <nav className="sticky top-0 z-50 bg-[var(--surface-color)] border-b border-orange-100 shadow-sm h-16">
      <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
        <Link href="/feed" className="flex items-center gap-3">
          ...
          <span className="hidden sm:block text-2xl font-bold text-[var(--primary-color)] serif-font tracking-tight">
            KalaSetu
          </span>
        </Link>
        <div className="hidden md:flex flex-1 max-w-xl mx-8 relative">
          <input
            ...
            type="text"
          />
          <span className="material-symbols-outlined absolute left-4 top-2.5 text-stone-400">
            search
          </span>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <Button
                        ...
                        className="text-xs text-[var(--primary-color)] hover:text-[var(--secondary-color)] font-medium transition-colors"
                      >
                        Mark all read
                      </Button>
                ...
                <div className="max-h-80 overflow-y-auto">
                  ...
                  {notifications.map((notif, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${kind.bg}`}
                        >
                          <span className={`material-symbols-outlined text-base ${kind.color}`}>
                            {kind.icon}
                          </span>
                        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-orange-100 p-4 space-y-2">
          {user && (
            <div className="flex items-center gap-3 px-4 py-3 mb-1 bg-orange-50 rounded-xl">
              <Avatar
                ...
                className="border-2 border-[var(--accent-color)]"
              />
              <div>
          ...
          >
            <span className="material-symbols-outlined">feed</span>Feed
          </Link>
          <Link
            href="/profile"
            className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-orange-50"
          ...
          <Link
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-orange-50"
          >
            <span className="material-symbols-outlined">event</span>Events & Gigs
          </Link>
          <Link
            href="/marketplace"
            className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-orange-50"
          ...
          >
            ...
            My Cart
            {cartCount > 0 && (
              <span className="ml-auto bg-[var(--primary-color)] text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                {cartCount}
              </span>
            )}
          </Link>
          ...
          <Link
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-orange-50"
          >
            <span className="material-symbols-outlined">savings</span>Funding
          </Link>
          <Link
            href="/settings"
            className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-orange-50"
          ...
          <Button
            onClick={handleLogout}
            className="flex items-center gap-3 px-4 py-3 rounded-lg text-red-500 hover:bg-red-50 w-full border-t border-stone-100 mt-1"
          >
