...
exports.search = catchAsync(async (req, res) => {
  ...
  if (!q) {
    return res
      .status(200)
      .json({ status: 'success', data: { artists: [], products: [], events: [] } });
  }
  ...
  if (!type || type === 'artists') {
    results.artists = await User.find({
      ...
      .select('fullName avatar title primaryArtForm location rating')
      .limit(10);
  }
  ...
  if (!type || type === 'events') {
    results.events = await Event.find({
      ...
      .select('title startDate venue category coverImage')
      .limit(10);
  }
