...
exports.bookTicket = catchAsync(async (req, res, next) => {
  ...
  if (event.eventType === 'paid') {
    if (event.soldQty + quantity > event.totalQty)
...
exports.rsvpEvent = catchAsync(async (req, res, next) => {
  ...
  if (event.maxAttendees && event.attendeeCount >= event.maxAttendees)
    return next(new AppError('Fully booked', 400));
...
exports.getAttendees = catchAsync(async (req, res, next) => {
  ...
  if (event.organizer.toString() !== req.user._id.toString())
    return next(new AppError('Not authorized', 403));
  const bookings = await Order.find({ event: event._id, orderType: 'booking' }).populate(
    'buyer',
