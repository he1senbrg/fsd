...
exports.getCampaigns = catchAsync(async (req, res) => {
  ...
  res
    .status(200)
    .json({ status: 'success', data: { campaigns: campaigns.map(serializeCampaign) } });
});
...
exports.backCampaign = catchAsync(async (req, res, next) => {
  ...
  await Order.create({
    ...
    buyer: req.user._id,
    orderType: 'crowdfunding',
    campaign: campaign._id,
    seller: campaign.creator,
    totalAmount: amount,
  ...
  if ([25, 50, 75, 100].includes(percent)) {
    NotificationService.notifyCampaignMilestone(
      campaign.creator,
      campaign._id,
      `${percent}% of its goal!`,
    ).catch(() => {});
...
exports.getBackers = catchAsync(async (req, res, next) => {
  ...
  const contributions = await Contribution.find({ campaign: campaign._id })
    .populate('backer', 'fullName avatar')
    .sort({ createdAt: -1 });
    ...
    data: {
      tiers: [
        { name: 'Rasa', amount: 500, benefits: ['Thank you note', 'Name in credits'] },
        {
          name: 'Kala',
          amount: 2000,
          benefits: ['All Rasa benefits', 'Exclusive updates', 'Early access'],
        },
        {
          name: 'Guru',
          amount: 5000,
          benefits: ['All Kala benefits', 'Meet the artist', 'Limited edition artwork'],
        },
      ],
    },
  });
