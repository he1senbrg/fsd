...
exports.createPost = catchAsync(async (req, res) => {
    ...
    const populatedPost = await Post.findById(post._id)
        .populate('author', 'fullName avatar title verified');
...
exports.toggleLike = catchAsync(async (req, res, next) => {
    ...
    if (existingLike) {
        ...
        res.status(200).json({ status: 'success', data: { isLiked: false, likeCount: updated.likeCount } });
    } else {
        ...
        res.status(200).json({ status: 'success', data: { isLiked: true, likeCount: updated.likeCount } });
    }
});
