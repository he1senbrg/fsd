...
exports.getTrendingOpportunities = catchAsync(async (req, res) => {
    const opps = await Opportunity.find({ status: 'open' })
        ...
        .sort({ applicationCount: -1 }).limit(3);
    res.status(200).json({ status: 'success', data: { opportunities: opps } });
});

exports.getOpportunity = catchAsync(async (req, res, next) => {
    const opp = await Opportunity.findById(req.params.id).populate('organizer', 'fullName avatar title location');
    if (!opp) return next(new AppError('Opportunity not found', 404));
    let hasApplied = false, isBookmarked = false;
    if (req.user) {
    ...
    res.status(200).json({ status: 'success', data: { opportunity: { ...opp.toObject(), hasApplied, isBookmarked } } });
});
...
exports.updateOpportunity = catchAsync(async (req, res, next) => {
    ...
    if (opp.organizer.toString() !== req.user._id.toString()) return next(new AppError('Not authorized', 403));
...
exports.applyToOpportunity = catchAsync(async (req, res, next) => {
    ...
    const application = await Application.create({
        opportunity: opp._id, applicant: req.user._id, coverLetter: req.body.coverLetter || '',
    });
...
exports.getApplications = catchAsync(async (req, res, next) => {
    ...
    const apps = await Application.find({ opportunity: opp._id })
        .populate('applicant', 'fullName avatar title primaryArtForm rating reviewCount');
