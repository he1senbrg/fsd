...
exports.getArtists = catchAsync(async (req, res) => {
  ...
  const artists = await User.find(filter)
    .select(
      'fullName avatar role title bio location primaryArtForm specializations rating reviewCount verified followerCount pricing',
    )
    .sort(sort);
...
exports.bookArtist = catchAsync(async (req, res, next) => {
  ...
  if (slot && slot.status === 'booked')
    return next(new AppError('Artist is not available on this date', 400));
